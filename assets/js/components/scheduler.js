...
function addEvent() {
    ...
    if($('.ae-from-input').val() && $('.ae-from-input').val().length !== 0){
        $('.hidden-clicked-date-value').attr('value', formatDate($('.ae-from-input').val()));
    }
    // if(!$('.ae-from-input').val())
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        dateClick: function (info) {
            $('.hidden-clicked-date-value').attr('value', formatDate(info.dateStr));
            dateInfo = info;
