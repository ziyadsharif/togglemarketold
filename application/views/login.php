...
<body>
    ...
    <script>
        ...
        function login() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response == 200) {
                        window.location = "<?php echo base_url("index.php/home/catalogue/"); ?>";
                        /*  $.ajax({
                             type: 'GET',
                        ...
                        response == 'Unauthorized' ? showFailureDialogue("The service is temporarily unavailable, please try again") :
                            showFailureDialogue(response);
        ...
        function forgotpassword() {
            $(".popup-wrapper").html('<div class="popup-container">' +
                '<div class="content-area">' +
                '<div class="login-modal">' +
                '<div class="logo">' +
                '<img src="<?php echo base_url() ?>assets/images/LOGOTRANS-1@2x.png" alt="Toggle market logo">' +
                ...
                '<div class="general-form">' +
                '<div class="formRow full no-margin-bottom">' +
                '<input type="email" id="InputForgotPasswordEmail" name="InputForgotPasswordEmail" placeholder="Username">' +
                '</div>' +
                '<div class="clearfix"></div>' +
                '<a href="<?php echo base_url("index.php/authenticate/signin"); ?>"logininstead()">Login instead</a>' +
                '<div class="clearfix"></div>' +
                '<div class="cta-holder">' +
                '<input type="button" class="cta-common" value="Submit" onclick="submitforgotpassword()">' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</div>' +
        ...
        showSuccessDialogue("The reset password link has been sent to your provided email")
        /*  $.ajax({
             type: 'POST',
             url: baseapiURL + 'saveemailid',
             data: JSON.stringify(qstrings),
             contentType: "application/json",
             dataType: 'json',
             success: function() {
        ...
        function forgotpasswordValidation(mymessage) {
